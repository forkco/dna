...
```css
--typeXlTitle2: 42px;   --typeXlTitle: 38px;    --typeLargeTitle: 34px;
...
All message anatomy rules, bubble specs, terminal geometry, attachment rules, and widget definitions
live in `pages/messages.html` — it is the single source of truth. Do not duplicate rules here.

Key structural rule: every message is **attachments → text → widget** (top to bottom, non-negotiable).
For full details, see the Message Anatomy and Attachment Rules sections in messages.html.
